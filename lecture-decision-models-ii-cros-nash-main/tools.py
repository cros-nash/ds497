...
def plot_asym_utility(power_utility_asym):
    plt.figure()
    x = np.linspace(-1000, 1000, 100)
    plt.plot(x, power_utility_asym(x, alpha=0.8, lambda_=0.5), label=r'$\lambda < 1$ (0.5)')
    plt.plot(x, power_utility_asym(x, alpha=0.8, lambda_=2), label=r'$\lambda > 1$ (2.0)')
    plt.plot(x, power_utility_asym(x, alpha=0.8, lambda_=1), label=r'$\lambda = 1$', color="black", lw=2)
    ...
    plt.xlabel('Money')
    plt.ylabel('Utility')
    # plt.title(r'Utility function $u(x) = x^{0.8}$, where $u(-x) = -\lambda x^{0.8}$')
    plt.legend()
    plt.grid(alpha=0.1)
    plt.show()

def plot_pw_curve(gamma=0.5):
    p = np.linspace(0, 1, 100)
    w = (p ** gamma) / ((p ** gamma) + ((1 - p) ** gamma)) ** (1 / gamma)
    plt.plot(p, w, label=f'γ={gamma}')
    plt.xlabel('p - Probability')
    plt.ylabel('π(p) - Decision Weight')
