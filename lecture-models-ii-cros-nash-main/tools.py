...
def predict_exemplar_props(learning_stims, test_data, c=1.0):
    """
    ...
    """
    # Get category A and B exemplars
    category_A = learning_stims[learning_stims['category'] == 'A'][['size', 'color']].values
    category_B = learning_stims[learning_stims['category'] == 'B'][['size', 'color']].values
    
    predictions = []
    
    # For each test stimulus
    for _, row in test_data.iterrows():
        stimulus = row[['size', 'color']].values
        
        # Sum similarities to category A exemplars
        sim_A = 0
        for exemplar in category_A:
            distance = np.sqrt(np.sum((stimulus - exemplar)**2))
            sim_A += np.exp(-c * distance)
            
        # Sum similarities to category B exemplars
        sim_B = 0
        for exemplar in category_B:
            distance = np.sqrt(np.sum((stimulus - exemplar)**2))
            sim_B += np.exp(-c * distance)
            
        # Convert to probability using choice rule
        prob_A = sim_A / (sim_A + sim_B)
        predictions.append(prob_A)
        
    return np.array(predictions)

def predict_prototype_props(learning_stims, test_data, c=1.0):
    """
    ...
    prototype_A = learning_stims[learning_stims['category'] == 'A'][['size', 'color']].values.mean(axis=0)
    prototype_B = learning_stims[learning_stims['category'] == 'B'][['size', 'color']].values.mean(axis=0)
    ...
    for _, row in test_data.iterrows():
        ...
        dist_A = np.sqrt(np.sum((stimulus - prototype_A)**2))
        dist_B = np.sqrt(np.sum((stimulus - prototype_B)**2))
        
        # Convert distances to similarities
        sim_A = np.exp(-c * dist_A)
        sim_B = np.exp(-c * dist_B)
